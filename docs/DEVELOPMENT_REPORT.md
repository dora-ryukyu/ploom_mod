...
> **現在の検証ステータス:** SuperLong **v5** — `st=8` 内で step0–6 をすべて **255s**（uint8 上限）に振り、**可能な限り長く**する天井テスト。温度は v4 据え置き。v4 実測 wall **~8:00** 確認済み。
...
| SuperLong 長さ天井 | **v5 検証中** | step0–6 = 255s 全振り（合計 1800s 設計） |
...
ユーザー方針: 温度強化より先に **前半も含め可能な限り長く**。  
`st=8`・uint8 のまま step0–6 をすべて **255s**、step07 は temp=0 終端のまま短く残す。温度は **v4 据え置き**。

| step | 時間 (s) | °C (v4) |
|------|----------|---------|
| 00–02 予熱 | **255+255+255** | 310 |
| 03 特殊 | **255** | −220 |
| 04 遷移 | **255** | 231 |
| 05 本加熱 | **255** | 250 |
| 06 後半 | **255** | 260 |
| 07 終端 | 15 | **0** |
| enableStep | 8 | |
| ステップ合計 | **1800s（30.0 min）** | |
| 単純換算 wall @0.86 | ~25.8 min（**保護で遥かに短く終わる可能性大**） | |

| 見る指標 | 読み方 |
|----------|--------|
| wall が v4 と同じ ~8 min | 本体に **隠れた総時間/セッション上限**がある |
| wall が伸びるが 30 分未満 | 上限 or 保護あり。実効天井が分かる |
| 異常終了・エラー | 255 全振りが非公式パターンとして拒否 |

**注意:** 公式は予熱を数分に抑える。255×3 の予熱は実験用の極値。
...
| **D5** | SuperLong 長さ天井（v5: 255×7） | **検証中** |
| **D6** | 温度再強化 / st=9 限界 | v5 結果後 |
...
1. **v5 実測**（1800s 設計がどこまで wall に効くか / 保護打ち切り）← **今ここ**
1b. v4 体感（後半パワー不足の主観は継続課題）
...
| `userHeatProfile/SuperLong.json` | **現行 v5** 定義 |
...
> **焼付は通る。step 時間は 255s 上限。v3/v4 で 543s→wall ~8 分。いま v5 で st=8 内の長さ天井（step0–6=255s、合計 1800s 設計）を実測中。**
